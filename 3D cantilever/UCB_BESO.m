%%%% A CODE OF BESO CONBINED WITH IDS(Gaussian) FOR 3D CANTILEVER BY SUN. H and MA. L %%%%
function [ITER,C,IOU,C_difference,xsubopt,nsubopt]=UCB_BESO(nelx,nely,nelz,volfrac,er,penal,rmin,E0,nu,rho,xsubopt);
...
nsubopt=[];
Sa = zeros(1,nele); Na = ones(1,nele);
...
while ~(vol <= volfrac && change < tolx) && loop < maxloop 
    ...
    if z > 1 && (vol-volfrac)>0.01
        dc0 = dc;
        index0 = find(first>=0);
        dc0(index0) = dc0(index0) + rho * sqrt(log(loop+1)/2./Na(index0));
        index1 = find(first==-1);
        dc0(index1) = max(max(max(dc0)));
        [x] = ADDDEL(nele,vol,dc0);  
    else
...
if z == 1
    ...
    first(index((1-volfrac/2)*nele+1:end))=-1;
end
%% evaluation
