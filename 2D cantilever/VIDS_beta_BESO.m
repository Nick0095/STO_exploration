%%%% A CODE OF BESO CONBINED WITH IDS(Beta) FOR 2D CANTILEVER BY SUN. H and MA. L %%%%
function [ITER,C,IOU,C_difference,xsubopt,nsubopt]=VIDS_BESO(nelx,nely,volfrac,er,rmin,E,nu,rate,xsubopt);
% INITIALIZE
iter = 0; rank=zeros(1000,nelx*nely); penal = 3.; M = 10000;
Nsubopt=10; 
...
n_arms = 10; 
beta = ones(1,n_arms); alpha = ones(1,n_arms); beta1 = ones(1,nely*nelx);

[rank0, qi, x, c]=BESO_Gaussian(nelx,nely,volfrac,er,rmin,E,nu);
data = rank0(1:qi,:); rank0 = data;
MAB_mu0 = mean(data);
[MAB_mu1,index]=sort(MAB_mu0);
P = 1:-1/nelx/nely:1/nelx/nely;   
beta1(index) = beta1(index) + qi * P * rate;
for i = 1:n_arms
    beta(i) = mean(beta1(index(80*50/n_arms*(i-1)+1:80*50/n_arms*i)));
end
alpha = 2 + qi * rate - beta;
...
beta1 = ones(1,nely*nelx); alpha1 = ones(1,nely*nelx);
...
%% thetas    
    if vol(i) > volfrac + 0.05
        ...
        MAB1 = rank(iter,:);
        for arm = 1:n_arms
            MAB(arm) = sum(x(index(80*50/n_arms*(arm-1)+1:80*50/n_arms*arm)).*...
                       MAB1(index(80*50/n_arms*(arm-1)+1:80*50/n_arms*arm)))/...
                       sum(x(index(80*50/n_arms*(arm-1)+1:80*50/n_arms*arm)));
            thetas(arm,:) = betarnd(alpha(arm),beta(arm),1,M);
        end
        beta1(index_dc) = beta1(index_dc) + P * rate;
        alpha1 = 2 + i * rate - beta1;
    end 
%% DESIGN UPDATE
    % calculate T
    if i == 1
    ...
    if vol(i) > volfrac + 0.05
        if T > 0
            [alpha,beta,arm_sequence,rem] = VID2(nelx,nely,N,n_arms,alpha,beta,thetas,M,T,MAB,rem,rate);
        end
        x = ones(nely,nelx);
        for j = 0.2*n_arms + 1:n_arms
            ...
            % reverse sampling 
            value = betarnd(alpha1(index(N*(j-1)+1:N*j)),beta1(index(N*(j-1)+1:N*j)));
...
function [alpha0,beta0,arm_sequence,rem] = VID2(nelx,nely,N,n_arms,alpha,beta,thetas,M,T,MAB,rem,rate)
rem(1:0.2*n_arms) = 0;
index0 = find(rem==0);
index1 = find(rem>0);
n_arms = length(index1);
thetas(index0,:) = []; MAB(:,index0) = [];
threshold = 0.99;
...
for t = 1:T
    % computeIDS
    [arm, p_a, optimal_arm] = computeVIDS(n_arms, threshold, Maap, p_a, thetas, M);
    % update_lists
    ...
    alpha0(index1(arm)) = alpha(arm); beta0(index1(arm)) = beta(arm);
    if rem(index1(arm)) == 0
        index1 = find(rem>0);
        n_arms = length(index1);
        Sa(arm) = []; Na(arm) = []; Maap(arm,:) = []; Maap(:,arm) = [];
        p_a(arm) = []; alpha(arm) = []; beta(arm) = []; MAB(:,arm) = [];
        thetas(arm,:) = [];         
    else
        thetas(arm,:) = betarnd(alpha(arm),beta(arm),1,M);
    end
